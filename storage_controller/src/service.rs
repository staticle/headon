...
impl Service {
    ...
    /// based on the logical size of the largest timeline on any of the tenant's shards. Uses the
    ...
    fn compute_split_shards(inputs: ShardSplitInputs) -> Option<ShardCount> {
        ...
        // Initial splits. Use larger of size-based and initial split shard counts. This only
...
mod tests {
    ...
    /// Tests Service::compute_split_shards. For readability, this specifies sizes in GBs
    /// rather than bytes.
    #[test]
    fn compute_split_shards() {
        // Size-based split: one shard has a 200 GB timeline -- this shard needs 2 splits into 4
        // shards, but there's already 2 shards to split, so the tenant splits into 8 shards.
        assert_eq!(
        ...
        // Size-based split: non-power-of-2 shard count is not normalized to powers-of-two. Instead
        // it's multiplied by 2. Concretely, a tenant with 3 shards splits twice into 6 then 12
        // shards, rather than into 4, then 8, then 16.
        assert_eq!(
            Service::compute_split_shards(ShardSplitInputs {
